        ...
        try{
           ...
           if(!isAvailable){
            ...
            const{data}=await axios.post('/api/bookings/book',
                {room: id,checkInDate,checkOutDate,guests,paymentMethod:"Pay At Hotel"},{headers:{Authorization: `Bearer ${await getToken()}`}})
            if(data.success){
        ...
        <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
          ...
          <form onSubmit={onSubmitHandler} className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.5)] p-6 rounded-xl mx-auto mt-16 max-w-6xl">
            ...
            <div className="flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500">
             ...
             text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer">{isAvailable ? "Book Now" : "Check Availabilty"}</button>
          </form>

          {/* {Common Specification} */}

          <div className="mt-25 space-y-4">{roomCommonData.map((spec,index)=>(
          ...
          <div>
            <p className="max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500">Guests will be allocated on the Ground floor as per availability. You get a comfortable Two bed apartment has a true city feeling. The price quoted is for two guest, at the guest slot please mark the number of guests to get the exact price for groups. 
